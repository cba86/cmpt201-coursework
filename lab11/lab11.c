...
#define MAX_FILE_SIZE 512
  ...
  size_t m_len = read_all_bytes(message_path, message, MAX_FILE_SIZE);
  size_t s_len = read_all_bytes(sign_path, signature, MAX_FILE_SIZE);

  // printf("DEBUG: Message: %s | Length: %zu\n", message_path, m_len);
  // printf("DEBUG: Signature: %s | Length: %zu\n", sign_path, s_len);
  ...
  // Feed message into hash
  if (EVP_DigestVerifyUpdate(ctx, message, m_len) <= 0) {
    EVP_MD_CTX_free(ctx);
    return -1;
  }

  // Compare result to provided signature
  int result = EVP_DigestVerifyFinal(ctx, signature, s_len);
