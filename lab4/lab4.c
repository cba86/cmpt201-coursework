#define _POSIX_C_SOURCE 200809L
#define _DEFAULT_SOURCE
#define _ISOC99_SOURCE
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>

#define HEAP_SIZE 256
#define BLOCK_SIZE 128
#define BUFF_SIZE 64

// keep track of size and ptr to next block (linked list)
struct header {
  uint64_t size;
  struct header *next;
};

// print out error message + exit
void error(const char *msg) {
  perror(msg);
  exit(EXIT_FAILURE);
}

// manual print function using write() instead of printf()
void print_out(char *format, void *data, size_t data_size) {
  char buff[BUFF_SIZE];

  // check if data is int (data_size == sizeof(uint64_t), ? true (cast to ptr +
  // dereference for value)
  //                                                      : false (cast to ptr
  //                                                      to ptr + deref for
  //                                                      ptr)
  ssize_t len =
      snprintf(buff, BUFF_SIZE, format,
               data_size == sizeof(uint64_t) ? *(uint64_t *)data
                                             : (uint64_t)(*(void **)data));

  if (len < 0) {
    error("snprint");
  }
  write(STDOUT_FILENO, buff, len);
}

// loop through block (skip header)
void print_block(char *start) {
  for (int i = 0; i < BLOCK_SIZE - sizeof(struct header); i++) {
    char *address = (start + sizeof(struct header) + i);
    // take byte + print value
    uint64_t val = (uint64_t)*address;
    print_out("%lu\n", &val, sizeof(val));
  }
}

// initialize block with header (size and ptr to next)
void initialize_block(struct header *block, uint64_t size, struct header *next,
                      int fill_value) {
  block->size = size;
  block->next = next;
  memset(block + 1, fill_value, size - sizeof(struct header));
}

int main() {
  // get current location of program break
  char *heap_start = sbrk(0);
  // move program break 256 bytes up
  if (sbrk(HEAP_SIZE) == (void *)-1) {
    error("sbrk");
  }

  struct header *first_block = (struct header *)heap_start;
  struct header *second_block = (struct header *)(heap_start + BLOCK_SIZE);

  // initialize first block (ptr->NULL, fill zero)
  initialize_block(first_block, BLOCK_SIZE, NULL, 0);
  // initialize second block (ptr->first_block, fill one)
  initialize_block(second_block, BLOCK_SIZE, first_block, 1);

  // print addresses
  print_out("first block: %p\n", &first_block, sizeof(first_block));
  print_out("second block: %p\n", &second_block, sizeof(second_block));

  // print sizes
  print_out("first block size: %lu\n", &first_block->size,
            sizeof(first_block->size));
  print_out("second block size: %lu\n", &second_block->size,
            sizeof(second_block->size));

  // print ptr next
  print_out("second block next: %p\n", &second_block->next,
            sizeof(second_block->next));

  // print content of first and second block
  print_block((char *)first_block);
  print_block((char *)second_block);

  return 0;
}
